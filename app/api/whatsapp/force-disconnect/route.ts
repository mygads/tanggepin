...
export async function POST(request: NextRequest) {
  ...
  const targetVillageId = body?.target_village_id
  
  if (!targetVillageId) {
    return NextResponse.json({ error: 'target_village_id diperlukan' }, { status: 400 })
  }

  const response = await fetch(
    `${CHANNEL_SERVICE_URL}/internal/whatsapp/force-disconnect?village_id=${encodeURIComponent(villageId)}`,
    {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ target_village_id: targetVillageId }),
    }
  )
